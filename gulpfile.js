	...
	const tasks = getCompilers('.scss').map((compile) => {
		...
		return gulp.src(compile.inputFile, {
			...
			.pipe(scss({
				...
				console.log('compile:scss.error', error);
			}))
			.pipe(autoprefixer())
...
gulp.task('watch', (done) => {
	if (getCompilers('.scss').length > 0) {
		gulp.watch('./src/**/*.scss', ['compile:scss']).on('change', log);
	}
	if (getCompilers('.js').length > 0) {
		gulp.watch('./src/**/*.js', ['compile:js']).on('change', log);
	}
	if (getCompilers('.ts').length > 0) {
		gulp.watch('./src/**/*.ts', ['compile:ts']).on('change', log);
	}
